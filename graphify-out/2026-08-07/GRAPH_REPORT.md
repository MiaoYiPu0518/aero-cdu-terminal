...
## Corpus Check
- 17 files · ~7,129 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 123 nodes · 149 edges · 12 communities (7 shown, 5 thin omitted)
...
4. `App()` - 6 edges
5. `CDUScreen()` - 3 edges
6. `eventToAnsi()` - 3 edges
7. `@xterm/addon-fit` - 2 edges
8. `@xterm/xterm` - 2 edges
9. `cors` - 2 edges
10. `express` - 2 edges
...
Cohesion: 0.16
Nodes (11): KeyProgrammerModal(), PRESET_PROFILES, ProfileModal(), TopBar(), ALTITUDES, CALLSIGNS, FACILITIES, FLIGHT_LEVELS (+3 more)
